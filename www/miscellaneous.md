...
The server will broadcast websocket events upon successful execution of any of the [authorized state changing routes](/list-of-routes/#authorized-routes), with the event name being the relative path (relative to the `ROOT_DIRECTORY_PATH` provided in `.env`) of the directory affected by the update. 
