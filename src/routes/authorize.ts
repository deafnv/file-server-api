...
router.post(
  '/register',
  body('username', 'Username must be between 4-16 characters long')
    .isString()
    .isLength({ min: 4, max: 16 }),
  body('password', 'Password must be between 6-255 characters long')
    .isString()
    .isLength({ min: 6, max: 255 }),
  validateErrors,
  async (req, res) => {
    if (!(dbEnabled && dbUsersEnabled)) return res.sendStatus(404)
...
router.patch('/user/:username/modify', authorize, async (req, res) => {
  if (!(dbEnabled && dbUsersEnabled)) return res.sendStatus(404)
  ...
  if (rank <= userToModifyData.rank && userToModify !== username)
    return res.status(403).send('User has equal or higher rank')
  if (payload.permissions && userToModify === username && notAdminRank)
    return res.status(403).send('You cannot modify your own permissions')
